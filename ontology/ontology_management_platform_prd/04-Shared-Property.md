...
### 2.1 产品目标
- 跨 Object Type 复用一致的 Property 语义和元数据
- 集中治理业务公共字段定义
- 降低跨领域字段口径漂移
- 为 Interface 和跨类型一致性提供基础

### 2.2 非目标 / 边界
- 不共享底层对象实例数据
- 不强制所有同名 Property 都转换为 Shared Property
- 不允许使用方私自修改全局元数据
...
## 5. 信息架构与页面设计
1. **Shared Properties 列表**
2. **Shared Property Detail**
3. **Consumers/Usage**
4. **Metadata & Type**
5. **Permissions**
6. **Versions/Changes**
...
| --- | --- | --- | --- | --- | --- |
| rid | ID | 是 | 不可变 | 全局唯一 | 资源标识 |
| name | String | 是 | 可编辑 | 建议唯一 | 业务名称 |
| apiName | String | 是 | 受限编辑 | 命名空间唯一 | 代码名 |
| description | Markdown | 否 | 可编辑 | 清晰定义语义 | 口径 |
| baseType | BaseType | 是 | 高风险编辑 | 受支持 | 技术类型 |
| valueTypeId | ValueTypeRef | 否 | 受控编辑 | Base Type兼容 | 共享验证语义 |
| valueFormatting | Object | 否 | 可编辑 | 按类型 | 统一展示 |
| typeClasses/capabilities | Object | 否 | 可编辑 | schema | 扩展能力 |
| renderHints | Object | 否 | 可编辑 | schema | 索引/渲染 |
| visibility | Enum | 是 | 可编辑 | 合法枚举 | 展示提示 |
| status | Enum | 是 | 可编辑 | experimental/active/deprecated | 生命周期 |
| owners | Principal[] | 是 | 可编辑 | 至少1 | 治理责任 |
...
| 需求ID | 优先级 | 需求说明 | 验收原则 |
| --- | --- | --- | --- |
| 04-FR-001 | P0 | 支持直接创建 Shared Property | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-002 | P0 | 支持从现有普通 Property 转换，并预填元数据 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-003 | P0 | Usage 页列出所有使用 Object Type、具体映射字段和状态 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-004 | P0 | 编辑共享元数据时预览对所有消费者的影响 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-005 | P0 | 高风险 Base Type/Value Type 变更必须阻断直接覆盖并要求新版本/迁移 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-006 | P0 | 消费者添加 Shared Property 时仅创建语义引用，仍需各自配置数据 mapping | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-007 | P0 | 支持 Owner/Steward 审批新的消费者接入（可配置） | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-008 | P0 | 支持推荐相似 Shared Property，减少重复创建 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-009 | P0 | 支持按业务域/Owner/Type/Status 搜索 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-010 | P0 | 支持一键生成影响分析报告 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-011 | P1 | 支持 deprecate 并引导消费者迁移 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 04-FR-012 | P1 | 支持与 Interface Property 的关系展示 | 服务端必须可验证；UI 需提供对应状态/反馈 |
...
| 规则ID | 级别 | 规则 |
| --- | --- | --- |
| 04-VAL-001 | Blocking | Base Type 变更必须检查所有消费者兼容性 |
| 04-VAL-002 | Blocking | API Name active 后受保护 |
| 04-VAL-003 | Blocking | 新增消费者的 Property mapping 类型必须兼容 |
| 04-VAL-004 | Blocking | 删除前 consumerCount 必须为0 |
| 04-VAL-005 | Blocking | Value Type 版本升级必须检查 constraint breaking |
| 04-VAL-006 | Blocking | 共享元数据的本地 override 默认不允许，必要时仅允许展示级 override |
...
- `PATCH /shared-properties/{id}`
- `POST /shared-properties/{id}:attach`
- `POST /shared-properties/{id}:deprecate`
- `GET /shared-properties/{id}/consumers`
### 15.1 API 通用约定
...
| 事件 | 触发时机 |
| --- | --- |
| sharedProperty.created | 对应资源状态或定义发生变化时 |
| sharedProperty.attached | 对应资源状态或定义发生变化时 |
| sharedProperty.metadata.changed | 对应资源状态或定义发生变化时 |
| sharedProperty.consumer.incompatible | 对应资源状态或定义发生变化时 |
| sharedProperty.deprecated | 对应资源状态或定义发生变化时 |
...
| 错误码示例 | 场景 | 用户提示 |
| --- | --- | --- |
| 04_VALIDATION_FAILED | 字段/结构校验失败 | 指出字段、规则和修复方式 |
| 04_VERSION_CONFLICT | 并发修改冲突 | 提示重新加载并展示差异 |
| 04_BREAKING_CHANGE_BLOCKED | active 资源发生阻断性变更 | 展示消费者和迁移入口 |
| 04_DEPENDENCY_EXISTS | 删除仍有依赖 | 列出可见依赖和处理建议 |
| 04_PERMISSION_DENIED | 无权限 | 不泄露敏感资源细节 |
| 04_REFERENCE_NOT_FOUND | 引用失效 | 指出失效引用并提供重新选择入口 |
...
| --- | --- | --- | --- | --- |
| 04-AC-001 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持直接创建 Shared Property | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-002 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持从现有普通 Property 转换，并预填元数据 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-003 | Happy Path | 已具备完成该功能所需权限和合法输入 | Usage 页列出所有使用 Object Type、具体映射字段和状态 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-004 | Happy Path | 已具备完成该功能所需权限和合法输入 | 编辑共享元数据时预览对所有消费者的影响 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-005 | Happy Path | 已具备完成该功能所需权限和合法输入 | 高风险 Base Type/Value Type 变更必须阻断直接覆盖并要求新版本/迁移 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-006 | Happy Path | 已具备完成该功能所需权限和合法输入 | 消费者添加 Shared Property 时仅创建语义引用，仍需各自配置数据 mapping | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-007 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持 Owner/Steward 审批新的消费者接入（可配置） | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-008 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持推荐相似 Shared Property，减少重复创建 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 04-AC-009 | Validation | 用户提交违反规则：Base Type 变更必须检查所有消费者兼容性 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 04-AC-010 | Validation | 用户提交违反规则：API Name active 后受保护 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 04-AC-011 | Validation | 用户提交违反规则：新增消费者的 Property mapping 类型必须兼容 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 04-AC-012 | Validation | 用户提交违反规则：删除前 consumerCount 必须为0 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 04-AC-013 | Validation | 用户提交违反规则：Value Type 版本升级必须检查 constraint breaking | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 04-AC-014 | Validation | 用户提交违反规则：共享元数据的本地 override 默认不允许，必要时仅允许展示级 override | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
