# Action Side Effects — PRD 级产品需求说明书
...
| 文档版本 | v1.0-PRD |
| 整理日期 | 2026-09-05 |
| 产品模块 | Action Side Effects |
| 元素分类 | Action Type 副作用子元素 |
...
## 2. 自研平台产品定位
**元素类别：Action Type 副作用子元素。** 本模块在自研 Ontology 管理平台中负责把 Palantir 的相关语义落实为可治理、可版本化、可审计、可通过 API 管理的产品能力。

### 2.1 产品目标
- 在 Action 成功/指定阶段触发通知、Webhook 或平台集成效果
- 让外部副作用可配置、可观测、可重试
- 避免业务系统在客户端自行拼接不可审计外部调用
- 为跨系统动作提供可靠性治理

### 2.2 非目标 / 边界
- Side Effect 不能替代 Ontology 主事务规则
- 不允许无限重试或无幂等策略的高风险外部写入
- 不在 side effect 中绕过安全和敏感信息控制
...
| --- | --- | --- | --- | --- | --- |
| id | ID | 是 | 不可变 | 唯一 | 标识 |
| type | Enum | 是 | 高风险编辑 | webhook/notification/... | 效果类型 |
| triggerPhase | Enum | 是 | 受控编辑 | after-success等 | 触发时点 |
| endpoint/connectionRef | Ref | 条件必填 | 受控编辑 | 连接有效 | 目标系统 |
| payloadTemplate | Template | 是 | 可编辑 | 引用合法 | 请求载荷 |
| headers | SecretAwareMap | 否 | 受控编辑 | 敏感值用SecretRef | 请求头 |
| timeout | Duration | 是 | 可编辑 | 平台范围内 | 超时 |
| retryPolicy | Object | 是 | 可编辑 | 次数/退避上限 | 重试 |
| idempotencyKey | Expression | 条件必填 | 可编辑 | 稳定可计算 | 幂等 |
| failureMode | Enum | 是 | 可编辑 | record/retry/compensate? | 失败处理 |
...
| 需求ID | 优先级 | 需求说明 | 验收原则 |
| --- | --- | --- | --- |
| 10-FR-001 | P0 | Webhook 配置必须通过受控 Connection/Secret，而不是明文 credential | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-002 | P0 | 支持 Test Send，使用测试上下文且默认不修改真实业务状态 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-003 | P0 | Payload Builder 可引用 action parameters、resulting edits、current user、execution metadata | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-004 | P0 | 执行日志记录 request metadata、status code、重试次数、最终状态，敏感字段脱敏 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-005 | P0 | 支持指数退避和最大重试次数 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-006 | P0 | 支持幂等键并在重复执行时复用/传递 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-007 | P0 | 明确主 Action 成功与 Side Effect 失败的事务边界 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-008 | P0 | 支持 dead-letter/人工重放机制（P1） | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-009 | P0 | 支持通知类 side effect 的收件人规则与模板预览 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 10-FR-010 | P0 | 所有 Side Effect 变更纳入版本和审计 | 服务端必须可验证；UI 需提供对应状态/反馈 |
...
| 规则ID | 级别 | 规则 |
| --- | --- | --- |
| 10-VAL-001 | Blocking | 外部 endpoint 必须来自允许的 Connection 或 allowlist |
| 10-VAL-002 | Blocking | secret 不得保存为普通字符串 |
| 10-VAL-003 | Blocking | 高风险写入 webhook 必须配置 idempotency key |
| 10-VAL-004 | Blocking | retryPolicy 不得无限重试 |
| 10-VAL-005 | Blocking | payload 引用必须存在且类型可序列化 |
| 10-VAL-006 | Blocking | 失败模式与 triggerPhase 组合必须合法 |
| 10-VAL-007 | Blocking | 敏感 Property 不得无授权写入外部 payload |
...
## 11. 生命周期与状态机
```text
Enabled -> Disabled -> DeliveryFailed
```
- 状态变更与内容变更分开授权。
...
## 15. API 需求
- `GET /action-types/{id}/side-effects`
- `POST /action-types/{id}/side-effects`
- `PATCH /side-effects/{id}`
- `POST /side-effects/{id}:test`
- `GET /side-effects/{id}/deliveries`
...
| 事件 | 触发时机 |
| --- | --- |
| sideEffect.created | 对应资源状态或定义发生变化时 |
| sideEffect.delivery.started | 对应资源状态或定义发生变化时 |
| sideEffect.delivery.succeeded | 对应资源状态或定义发生变化时 |
| sideEffect.delivery.failed | 对应资源状态或定义发生变化时 |
| sideEffect.replayed | 对应资源状态或定义发生变化时 |
...
| 错误码示例 | 场景 | 用户提示 |
| --- | --- | --- |
| 10_VALIDATION_FAILED | 字段/结构校验失败 | 指出字段、规则和修复方式 |
| 10_VERSION_CONFLICT | 并发修改冲突 | 提示重新加载并展示差异 |
| 10_BREAKING_CHANGE_BLOCKED | active 资源发生阻断性变更 | 展示消费者和迁移入口 |
| 10_DEPENDENCY_EXISTS | 删除仍有依赖 | 列出可见依赖和处理建议 |
| 10_PERMISSION_DENIED | 无权限 | 不泄露敏感资源细节 |
| 10_REFERENCE_NOT_FOUND | 引用失效 | 指出失效引用并提供重新选择入口 |
...
| --- | --- | --- | --- | --- |
| 10-AC-001 | Happy Path | 已具备完成该功能所需权限和合法输入 | Webhook 配置必须通过受控 Connection/Secret，而不是明文 credential | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-002 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持 Test Send，使用测试上下文且默认不修改真实业务状态 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-003 | Happy Path | 已具备完成该功能所需权限和合法输入 | Payload Builder 可引用 action parameters、resulting edits、current user、execution metadata | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-004 | Happy Path | 已具备完成该功能所需权限和合法输入 | 执行日志记录 request metadata、status code、重试次数、最终状态，敏感字段脱敏 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-005 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持指数退避和最大重试次数 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-006 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持幂等键并在重复执行时复用/传递 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-007 | Happy Path | 已具备完成该功能所需权限和合法输入 | 明确主 Action 成功与 Side Effect 失败的事务边界 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-008 | Happy Path | 已具备完成该功能所需权限和合法输入 | 支持 dead-letter/人工重放机制（P1） | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 10-AC-009 | Validation | 用户提交违反规则：外部 endpoint 必须来自允许的 Connection 或 allowlist | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 10-AC-010 | Validation | 用户提交违反规则：secret 不得保存为普通字符串 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 10-AC-011 | Validation | 用户提交违反规则：高风险写入 webhook 必须配置 idempotency key | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 10-AC-012 | Validation | 用户提交违反规则：retryPolicy 不得无限重试 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 10-AC-013 | Validation | 用户提交违反规则：payload 引用必须存在且类型可序列化 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 10-AC-014 | Validation | 用户提交违反规则：失败模式与 triggerPhase 组合必须合法 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
