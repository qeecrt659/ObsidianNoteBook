# Ontology Security & Permissions — PRD 级产品需求说明书
...
| 文档版本 | v1.0-PRD |
| 整理日期 | 2026-09-05 |
| 产品模块 | Ontology Security & Permissions |
| 元素分类 | 横切 Data / Logic / Action 的安全治理能力 |
...
## 2. 自研平台产品定位
**元素类别：横切 Data / Logic / Action 的安全治理能力。** 本模块在自研 Ontology 管理平台中负责把 Palantir 的相关语义落实为可治理、可版本化、可审计、可通过 API 管理的产品能力。

### 2.1 产品目标
- 统一治理资源级、对象级、属性级和 Action 执行权限
- 支持 row/column/cell-level security
- 提供策略测试、可解释性和审计
- 确保读取后的外部传播边界被明确提示

### 2.2 非目标 / 边界
- Visibility/hidden 不等于权限
- 前端禁用按钮不等于授权
- 对象/属性 read-time policy 不自动保证导出后的所有下游传播安全
...
| --- | --- | --- | --- | --- | --- |
| policyId | ID | 是 | 不可变 | 唯一 | 策略标识 |
| policyType | Enum | 是 | 不可变/高风险 | resource/object/property/action | 类型 |
| resourceRef | Ref | 是 | 受控 | 存在 | 目标资源 |
| principalScope | Expression | 是 | 可编辑 | 有效principal | 主体 |
| condition | Expression | 条件必填 | 可编辑 | 可执行 | 行/列条件 |
| effect | Enum | 是 | 可编辑 | allow/deny/filter等按模型 | 效果 |
| priority | Integer | 否 | 受控 | 无冲突 | 求值顺序 |
| mandatoryControls | Ref[] | 否 | 受控 | 有效 | marking/org/classification |
| status | Enum | 是 | 可编辑 | draft/active/deprecated | 生命周期 |
...
| 需求ID | 优先级 | 需求说明 | 验收原则 |
| --- | --- | --- | --- |
| 16-FR-001 | P0 | 资源级权限控制查看/编辑 Object/Link/Action/Interface 等定义 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-002 | P0 | Object Security Policy 支持对象行级过滤 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-003 | P0 | Property Security Policy 支持字段级过滤并与对象策略组合形成 cell-level | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-004 | P0 | 禁止把 Primary Key 放入 Property Security Policy | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-005 | P0 | Action 执行权限与 Action 定义编辑权限分离 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-006 | P0 | Policy Tester 输入 User + Object/Action Context，解释最终可见字段/是否可执行 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-007 | P0 | Access Check 显示权限链条和失败节点 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-008 | P0 | 所有策略发布需要变更审计和可选审批 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-009 | P0 | 敏感字段的日志/导出/UI 预览必须遵守策略 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-010 | P0 | 文档明确 read-time enforcement 与下游传播边界 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-011 | P1 | 支持 Marking/Organization/Classification 等强制控制的集成点 | 服务端必须可验证；UI 需提供对应状态/反馈 |
| 16-FR-012 | P1 | 策略模拟不得真正执行 Action/Side Effect | 服务端必须可验证；UI 需提供对应状态/反馈 |
...
| 规则ID | 级别 | 规则 |
| --- | --- | --- |
| 16-VAL-001 | Blocking | Primary Key 不允许 Property policy 隐藏 |
| 16-VAL-002 | Blocking | 同一非 PK Property 的 policy 组合规则必须确定且无歧义 |
| 16-VAL-003 | Blocking | Action 后端必须在执行点重新授权 |
| 16-VAL-004 | Blocking | policy expression 必须可编译 |
| 16-VAL-005 | Blocking | Owner 不能删除自己的最后一个治理路径（break-glass除外） |
| 16-VAL-006 | Blocking | 策略变更不得导致平台管理服务自身不可恢复锁死 |
| 16-VAL-007 | Blocking | 测试结果不得泄露用户无权读取的实际数据 |
...
- `POST /security/policies/{id}:test`
- `POST /security/check-access`
...
| 事件 | 触发时机 |
| --- | --- |
| security.policy.created | 对应资源状态或定义发生变化时 |
| security.policy.changed | 对应资源状态或定义发生变化时 |
| security.access.denied | 对应资源状态或定义发生变化时 |
| security.policy.tested | 对应资源状态或定义发生变化时 |
| security.breakGlass.used | 对应资源状态或定义发生变化时 |
...
| 错误码示例 | 场景 | 用户提示 |
| --- | --- | --- |
| 16_VALIDATION_FAILED | 字段/结构校验失败 | 指出字段、规则和修复方式 |
| 16_VERSION_CONFLICT | 并发修改冲突 | 提示重新加载并展示差异 |
| 16_BREAKING_CHANGE_BLOCKED | active 资源发生阻断性变更 | 展示消费者和迁移入口 |
| 16_DEPENDENCY_EXISTS | 删除仍有依赖 | 列出可见依赖和处理建议 |
| 16_PERMISSION_DENIED | 无权限 | 不泄露敏感资源细节 |
| 16_REFERENCE_NOT_FOUND | 引用失效 | 指出失效引用并提供重新选择入口 |
...
| --- | --- | --- | --- | --- |
| 16-AC-001 | Happy Path | 已具备完成该功能所需权限和合法输入 | 资源级权限控制查看/编辑 Object/Link/Action/Interface 等定义 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-002 | Happy Path | 已具备完成该功能所需权限和合法输入 | Object Security Policy 支持对象行级过滤 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-003 | Happy Path | 已具备完成该功能所需权限和合法输入 | Property Security Policy 支持字段级过滤并与对象策略组合形成 cell-level | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-004 | Happy Path | 已具备完成该功能所需权限和合法输入 | 禁止把 Primary Key 放入 Property Security Policy | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-005 | Happy Path | 已具备完成该功能所需权限和合法输入 | Action 执行权限与 Action 定义编辑权限分离 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-006 | Happy Path | 已具备完成该功能所需权限和合法输入 | Policy Tester 输入 User + Object/Action Context，解释最终可见字段/是否可执行 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-007 | Happy Path | 已具备完成该功能所需权限和合法输入 | Access Check 显示权限链条和失败节点 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-008 | Happy Path | 已具备完成该功能所需权限和合法输入 | 所有策略发布需要变更审计和可选审批 | 系统完成操作并持久化变更；审计日志可查询；刷新页面结果一致 |
| 16-AC-009 | Validation | 用户提交违反规则：Primary Key 不允许 Property policy 隐藏 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 16-AC-010 | Validation | 用户提交违反规则：同一非 PK Property 的 policy 组合规则必须确定且无歧义 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 16-AC-011 | Validation | 用户提交违反规则：Action 后端必须在执行点重新授权 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 16-AC-012 | Validation | 用户提交违反规则：policy expression 必须可编译 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 16-AC-013 | Validation | 用户提交违反规则：Owner 不能删除自己的最后一个治理路径（break-glass除外） | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
| 16-AC-014 | Validation | 用户提交违反规则：策略变更不得导致平台管理服务自身不可恢复锁死 | 点击保存/发布/执行 | 系统拒绝请求并返回明确 errorCode 与修复建议，不产生部分写入 |
